...
async def main():
    ...
    parser.add_argument("--save_fn", type=str, default='results/toul_1209_emo_adr_re_n')
                ...
                for i in range(0, rounds):
                    ...
                    if "no" not in res.lower():
                        ...
                        if len(disagr_bank) != 0:
                            agr_bank.append(disagr_bank[int(res)])
                            del disagr_bank[int(res)]
    ...
    print(len(anas), len(conversation_teacher), len(conversation_student), len(lm_thought), len(sums), len(reles))
    data_dict = {
